...
export function isCorrect(question: Question, answer: string): boolean {
    ...
    const compareAnswer = answer.trim().toLowerCase();

    const isCorrect = correctAnswer === compareAnswer;
...
export function toShortForm(question: Question): string {
    ...
    const shortenName = questionName.slice(0, 10);

    const formatedQues = questionID + ": " + shortenName;
...
export function toMarkdown(question: Question): string {
    const quesName = question.name;
    const quesBody = question.body;
    const quesOptions = question.options;

    if (quesOptions.length > 0) {
        const formatedOptions = "- " + quesOptions.join("\n- ");

        const markDown = `# ${quesName}\n${quesBody}\n${formatedOptions}`;
        return markDown;
    } else {
        const markDown = `# ${quesName}\n${quesBody}`;
        return markDown;
